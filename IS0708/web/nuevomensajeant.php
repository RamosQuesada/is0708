...
<div id="wrap">
		...
		<div id="content-wrap">
			...
			<div id="main">
				...
				{
					$sql="select numvendedor,nombre,apellido1,apellido2 from usuario where numvendedor in (select numvendedor from departamentousuario where nombredepartamento in (select nombredepartamento from departamentoUsuario where numvendedor=".$_SESSION['codigo']."))";
					...
					{
						echo "<tr align=left><td>Destinatario</td><td><select name=destinatario width=100>";
						for ($i=0;$i<$nreg;$i++)
						{
...
$registrob=$db->query($sql);
$nreg2=$registrob->num_rows;
for ($i=0;$i<$nreg2;$i++)
{
	$reg2=$registrob->fetch_assoc();
	echo "<option value=".$reg2['JefeDepartamento'].">".$reg2['JefeDepartamento']." -- Jefe Departamento = ".$reg2['Nombre']."</option>";
}
