 ...
 */	
	...
	private int despPestana1 = 0;
	private int despPestana2 = 0;
	private boolean subiendoOBajandoPestanas = false;
	
	private void subePestana1() {
		if (despPestana1<13) {
			despPestana1++;
	...
	public boolean getEstadoPestana() {
		return subiendoOBajandoPestanas;
	}
	...
	public void dibujar(Display display, String nombre, GC gc, int posV, Color color, int margenIzq, int margenNombres, int margenSup, int sep_vert_franjas, int alto_franjas, int tamHora, int tamSubdiv, int horaApertura, int numSubdiv, int y) {
		...
		if (activa1 || despPestana1!=0) {
			...
			gc.drawText(s, inicio1+5, despV-despPestana1-1, true);
		...
		if (activa2 || despPestana2!=0) {
			...
			Util.cambiarRelleno(display, gc, r,g,b);	
			gc.fillRoundRectangle(inicio2, despV-despPestana2-2, 135, 20,8,8);
			gc.drawRoundRectangle(inicio2, despV-despPestana2-2, 135, 20,8,8);
