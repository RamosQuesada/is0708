		...
		bNuevoContrato.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				I09_1_Creacion_contratos i09 = new I09_1_Creacion_contratos(
						tabFolder.getShell(), bundle, vista, 0, -1, null);
				...
				if (resp == SWT.OK) {
					if (tablaContratos.getSelectionIndex() > -1) {
						...
						I09_1_Creacion_contratos i09c = new I09_1_Creacion_contratos(
								tabFolder.getShell(), bundle, vista, 1, Integer
										.parseInt(it.getText(0)), vista
										.getListaContratosDepartamento().get(
												tablaContratos
														.getSelectionIndex()));
					...
					if (tablaContratos.getSelectionIndex() > -1) {
						if (!tablaContratos.getItem(
							...
							if (response == SWT.OK) {
								...
								//eliminar los turnos que no pertenecen a otro contrato
								...
								ArrayList<Turno> turnosOut=vista.getControlador().getTurnosDeUnContrato(idOut);
								int apariciones=0;
								for (int k=0;k<turnosOut.size();k++){
									int idEliminado=turnosOut.get(k).getIdTurno();
									...
									if (apariciones==0) turnosFuera.add(idEliminado);
								}
								boolean okis=true;
								if (turnosFuera.size()==0) okis=true;
								else {
									for (int i=0;i<turnosFuera.size();i++)
										okis = okis && vista.eliminaTurno(turnosFuera.get(i));
								}
								// CAMBIAR
