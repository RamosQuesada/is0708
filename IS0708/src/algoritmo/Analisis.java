					...
					for (int k=0;k<60;k=k+5){
						...
						if (contador<minimoDia){
							if ((sugAnterior!=null)&&(minimoDia-contador!=sugAnterior.getFaltas()) && (sugAnterior.getTipo()!=tipoFalta(contador,minimoDia))){		
								Sugerencia sug=new Sugerencia(minimoDia-contador,minimoDia,new Time(t.getHours(),t.getMinutes(),0),new Time(t.getHours(),t.getMinutes()+5,0),new Date (cuadrante.getAnio()+1900,cuadrante.getMes()-1,i+1),tipoFalta(contador,minimoDia));
